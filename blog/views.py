from django.shortcuts import render
from django.views.generic import ListView, DetailView
from content.models import Entry, EntryTag
from django.utils.safestring import mark_safe
from bs4 import BeautifulSoup
from django.core.paginator import Paginator
from django.shortcuts import render, get_object_or_404


POSTS_PER_PAGE = 10
RECENT_POSTS = 6

class PostListView(ListView):
    model = Entry
    template_name = "blog/index.html"
    context_object_name = "posts"
    paginate_by = POSTS_PER_PAGE

    def get_queryset(self):
        return Entry.objects.filter(type='post', status='published').order_by('-created_at')

    def get_context_data(self, **kwargs):
        context = super().get_context_data(**kwargs)

        # --- Recent posts ---
        context["recent_posts"] = Entry.objects.filter(type="post", status="published").order_by("-created_at")[:RECENT_POSTS]

        # --- Tags used by published posts ---
        post_qs = Entry.objects.filter(type='post', status='published')
        context["all_tags"] = EntryTag.objects.filter(entry_tags__in=post_qs).distinct().order_by('name')

        return context


class PostDetailView(DetailView):
    model = Entry
    template_name = "blog/post_detail.html"
    context_object_name = "post"

    def get_queryset(self):
        return Entry.objects.filter(type='post', status='published')

    def get_context_data(self, **kwargs):
        context = super().get_context_data(**kwargs)
        post = context["post"]

        # --- TOC generation ---
        soup = BeautifulSoup(post.body, "lxml")
        toc_headings = []
        for index, tag in enumerate(soup.find_all(["h2", "h3"])):
            tag_id = tag.get("id") or f"section-{index}"
            tag["id"] = tag_id
            toc_headings.append({
                "text": tag.get_text(),
                "id": tag_id,
                "level": tag.name
            })
        post.body = mark_safe(str(soup))
        context["toc_headings"] = toc_headings

        # --- Recent posts ---
        context["recent_posts"] = Entry.objects.filter(type="post", status="published").order_by("-created_at")[:RECENT_POSTS]

        # --- Tags used by published posts ---
        post_qs = Entry.objects.filter(type='post', status='published')
        context["all_tags"] = EntryTag.objects.filter(entry_tags__in=post_qs).distinct().order_by('name')

        return context



# -----------------------------
# Search posts by title only
# -----------------------------
def post_search(request):
    query = request.GET.get('q', '').strip()

    # Base queryset
    post_list = Entry.objects.filter(
        type='post',
        status__in=['published']
    ).select_related('author').prefetch_related('tags', 'links')

    # Filter by title query
    if query:
        post_list = post_list.filter(title__icontains=query)

    # Final ordering
    post_list = post_list.order_by('-created_at')

    # Pagination
    paginator = Paginator(post_list, POSTS_PER_PAGE)
    page_number = request.GET.get('page')
    page_obj = paginator.get_page(page_number)

    # Get all tags for sidebar
    all_tags = EntryTag.objects.filter(entry_tags__in=post_list).distinct().order_by('name')

    context = {
        'query': query,
        'page_obj': page_obj,
        'all_tags': all_tags,
    }
    return render(request, 'blog/search_results.html', context)


# -----------------------------
# Filter posts by tag
# -----------------------------
def post_tag(request, slug):
    current_tag = get_object_or_404(EntryTag, slug=slug)

    post_list = (
        Entry.objects.filter(
            type='post',
            status='published',
            tags=current_tag,
        )
        .select_related('author')
        .prefetch_related('tags', 'links')
        .order_by('-created_at')
    )

    # Pagination
    paginator = Paginator(post_list, POSTS_PER_PAGE)
    page_number = request.GET.get('page')
    page_obj = paginator.get_page(page_number)

    # All tags for sidebar
    all_tags = EntryTag.objects.filter(entry_tags__in=Entry.objects.filter(type='post', status='published')).distinct().order_by('name')

    context = {
        'page_obj': page_obj,
        'current_tag': current_tag,
        'all_tags': all_tags,
    }
    return render(request, 'blog/tag.html', context)














